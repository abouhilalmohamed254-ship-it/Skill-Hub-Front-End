...
export default function CatalogueCard({ course }: { course: Course }) {
  return (
    <div className="group relative h-full bg-gradient-to-br from-white/80 to-neutral-50/50 dark:from-neutral-800/50 dark:to-neutral-900/30 backdrop-blur-sm border border-neutral-200/60 dark:border-neutral-700/60 rounded-2xl overflow-hidden shadow-md dark:shadow-black/30 hover:shadow-lg dark:hover:shadow-black/50 transition-all duration-200">
      <div className="relative p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="text-xl font-bold text-neutral-900 dark:text-neutral-50 leading-tight line-clamp-2 flex-1">
            {course.title}
          </h3>
          <div className={`${levelBadges[course.level] ?? ""} rounded-lg px-2 py-1 text-xs font-bold tracking-wide whitespace-nowrap`}>
            {course.level}
          </div>
        </div>
        ...
        <div className="space-y-2 mb-4 pb-4 border-b border-neutral-200/50 dark:border-neutral-700/50">
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-neutral-500 dark:text-neutral-500">Duration</span>
            <span className="font-medium text-neutral-900 dark:text-neutral-100">{course.duration}h</span>
          </div>
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-neutral-500 dark:text-neutral-500">Certification</span>
            <span className="font-medium text-neutral-900 dark:text-neutral-100">{course.certification}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="text-2xl font-bold text-neutral-900 dark:text-neutral-50">
            {course.price}
            <span className="text-sm font-normal text-neutral-600 dark:text-neutral-400 ml-1">DH</span>
          </div>
          <Link
