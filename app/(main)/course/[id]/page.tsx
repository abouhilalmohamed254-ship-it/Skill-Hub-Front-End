    ...
    <div className="h-full overflow-y-auto p-4 lg:p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 lg:mb-8">
          <h1 className="text-3xl lg:text-4xl font-bold text-neutral-900 dark:text-neutral-50">Course Details</h1>
          <Link
            className="px-6 py-3 bg-white/50 dark:bg-neutral-800/30 text-neutral-900 dark:text-neutral-50 rounded-xl font-semibold border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-200/70 dark:hover:bg-neutral-700/50 hover:scale-105 transition-all duration-200 flex items-center gap-2"
            href="/catalogue"
          >
        ...
        <div className="bg-white/80 dark:bg-neutral-800/50 backdrop-blur-sm border border-neutral-200/60 dark:border-neutral-700/60 rounded-2xl shadow-sm dark:shadow-black/20 p-5 lg:p-6 mb-5">
          <div className="flex items-start justify-between gap-4 mb-6">
            <div className="flex-1">
              <h2 className="text-2xl lg:text-3xl font-bold text-neutral-900 dark:text-neutral-50 mb-2">{course.title}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { label: "Price", value: `${course.price}`, suffix: "DH" },
              { label: "Duration", value: `${course.duration}`, suffix: "hours" },
              { label: "Students Enrolled", value: `${course.studentsNumber}` },
              { label: "Certification", value: course.certification === "Certificated" ? "Yes" : "No" },
              { label: "Status", value: course.status },
            ].map((item) => (
              <div key={item.label} className="bg-gradient-to-br from-white/50 to-neutral-50/30 dark:from-neutral-900/30 dark:to-neutral-800/20 p-4 rounded-xl border border-neutral-200/40 dark:border-neutral-700/40">
                <div className="text-xs font-semibold text-neutral-500 dark:text-neutral-500 uppercase tracking-wide mb-1">{item.label}</div>
